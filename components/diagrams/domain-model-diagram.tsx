...
const stepVisibility = {
  1: { concepts: [] as string[], associations: [] as string[], showAttributes: false, showAssociationDetails: false },
  2: { concepts: ["student", "organization", "event"], associations: [], showAttributes: false, showAssociationDetails: false },
  3: { concepts: ["student", "organization", "event", "membershipRequest", "membership", "rsvp"], associations: [], showAttributes: true, showAssociationDetails: false },
  4: { concepts: ["student", "organization", "event", "membershipRequest", "membership", "rsvp"], associations: ["student-membership", "membership-org", "org-event"], showAttributes: true, showAssociationDetails: false },
  5: { concepts: ["student", "organization", "event", "membershipRequest", "membership", "rsvp"], associations: ["student-membership", "membership-org", "org-event", "student-request", "request-org", "student-rsvp", "rsvp-event"], showAttributes: true, showAssociationDetails: true },
  6: { concepts: ["student", "organization", "event", "membershipRequest", "membership", "rsvp"], associations: ["student-membership", "membership-org", "org-event", "student-request", "request-org", "student-rsvp", "rsvp-event"], showAttributes: true, showAssociationDetails: true },
};
...
export function DomainModelDiagram({ currentStep }: DomainModelDiagramProps) {
  ...
  useEffect(() => {
    ...
    if (prevStep === null || step > prevStep) {
      ...
      const prev = prevStep ? stepVisibility[prevStep as keyof typeof stepVisibility] : { concepts: [] as string[], associations: [] as string[], showAttributes: false, showAssociationDetails: false };
      ...
      if (current.showAssociationDetails && !prev.showAssociationDetails) newAnimating.add("assoc-details");
    }
    ...
    if (newAnimating.size > 0) {
      const CONCEPT_MS = 520;
      const LINE_MS = 700;
      const ATTR_MS = 460;
      const DETAILS_MS = 460;
      const CONCEPT_STAGGER = 100;
      const ASSOC_STAGGER = 80;
      const ATTR_DELAY = 150;
      const ATTR_STAGGER = 40;
      const DETAILS_DELAY = 120;
      const DETAILS_STAGGER = 30;
      ...
      for (const key of newAnimating) {
        if (key.startsWith("concept-")) {
          ...
          const delay = Math.max(0, idx) * CONCEPT_STAGGER;
          maxMs = Math.max(maxMs, delay + CONCEPT_MS);
          continue;
        }

        if (key.startsWith("assoc-")) {
          const id = key.slice("assoc-".length);
          const idx = associations.findIndex(a => a.id === id);
          const delay = Math.max(0, idx) * ASSOC_STAGGER;
        ...
        if (key === "attributes") {
          const current = stepVisibility[step];
          const maxDelay = current.concepts.reduce((acc, id) => {
            const idx = concepts.findIndex(c => c.id === id);
        ...
        if (key === "assoc-details") {
          const current = stepVisibility[step];
          const maxDelay = current.associations.reduce((acc, id) => {
            const idx = associations.findIndex(a => a.id === id);
            return Math.max(acc, DETAILS_DELAY + Math.max(0, idx) * DETAILS_STAGGER);
          }, 0);
          maxMs = Math.max(maxMs, maxDelay + DETAILS_MS);
          continue;
        }
      }
  ...
  const showAssociationDetails = showAll || (visibility?.showAssociationDetails ?? false);
      ...
      <div className="w-full overflow-x-auto">
        ...
        >
          ...
          {associations.map((assoc, index) => {
            ...
            const showDetailsAnimating = isAnimating("assoc-details");
            ...
            return (
              <g
                key={assoc.id}
                style={animating ? { animationDelay: `${index * 80}ms` } : undefined}
              >
                <line
                  x1={from.x}
                  y1={from.y}
                ...
                {showAssociationDetails && (
                  <g
                    className={showDetailsAnimating ? "animate-fade" : ""}
                    style={showDetailsAnimating ? { animationDelay: `${120 + index * 30}ms` } : undefined}
                  >
                    <text
                      x={midX}
                      y={midY - 6}
          ...
          {concepts.map((concept, index) => {
            ...
            const totalHeight = showAttributes ? 28 + attrHeight : 28;
                ...
                >
                  ...
                  {showAttributes && (
                    <g
                      ...
                      style={attrsAnimating ? { animationDelay: `${150 + index * 40}ms` } : undefined}
                    >
